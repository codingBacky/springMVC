...
public class BoardDAO {
	...
	public int getListCount() {
		...
		try{
			...
			pstmt=con.prepareStatement("select count(*) boardCnt from board");
	...
	public List<BoardBean> getBoardList(int page,int limit){
		...
		int startrow=(page-1)*10+1; //시작 한페이지에 10개씩 보기
	...
	public boolean boardInsert(BoardBean board){
		...
		try{
			...
			sql+="BOARD_CONTENT, BOARD_FILE, BOARD_RE_REF,"+
				...
				"BOARD_DATE) values(board_seq.nextval,?,?,?,?,?,board_seq.currval,?,?,?,sysdate)";
	...
	//쿼리문만 실행하도록 리펙토링 해줘야함, 나머지는 replyAction에서 해서 DAO에게 일을 시켜야함
	public int boardReply(BoardBean board){
		...
		int re_ref=board.getBOARD_RE_REF();//hidden으로 넘어온 자료를 그대로 넘겨줌
